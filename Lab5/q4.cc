...
void experiment (bool enableCtsRts, std::string wifiManager)
{
 ...
 //First create the loss matrix object called "lossModel"
  ...
  // set symmetric loss 2 <-> 3 to 50 dB, good coverage
 ...
 //With this statement we are assuming that 10.0.0.1 as the IP address of node "n0"
  OnOffHelper onOffHelperb ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address ("10.0.0.2"), cbrPort));
  OnOffHelper onOffHelperd ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address ("10.0.0.4"), cbrPort));
  ...
  //OnOff source can be set to just generate packets at some data rate)
   ...
   onOffHelperd.SetConstantRate(DataRate (dataRate), payloadSize);
  ...
  //Now Install this helper object on ni, and add the returned object, an application to the cbrApps container
	 ...
	 cbrApps.Add (onOffHelperb.Install (nodes.Get (0)));
   ...
   */
	 ...
	 onOffHelperd.SetAttribute ("StartTime", TimeValue (Seconds (stime)));
	 cbrApps.Add (onOffHelperd.Install (nodes.Get (2)));
   ...
   */
  uint16_t  echoPort = 9;
  UdpEchoClientHelper echoClientHelperb (Ipv4Address ("10.0.0.2"), echoPort);
  ...
  UdpEchoClientHelper echoClientHelperd (Ipv4Address ("10.0.0.4"), echoPort);
  echoClientHelperd.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClientHelperd.SetAttribute ("Interval", TimeValue (Seconds (0.1)));
  echoClientHelperd.SetAttribute ("PacketSize", UintegerValue (10));
  ApplicationContainer pingAppsd;

  // again using different start times to workaround Bug 388 and Bug 912
  echoClientHelperb.SetAttribute ("StartTime", TimeValue (Seconds ((double)1/1000)));
  pingAppsb.Add (echoClientHelperb.Install (nodes.Get (0)));

echoClientHelperd.SetAttribute ("StartTime", TimeValue (Seconds ((double)2/1000)));
  pingAppsd.Add (echoClientHelperd.Install (nodes.Get (2)));
